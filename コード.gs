...
var left=["",0,4,21,28,52,76,100,124,148,172,196,220];

var k="1hMGAeQY_o2ZvqJuEmudDVVrota-R333zM2Cq3GNamKk"
function doGet() {
  var ss = SpreadsheetApp.openById(SCRIPT_PROP.getProperty("key")); //SpreadsheetApp.openById(sid);
  //var ss = SpreadsheetApp.openById(k); //SpreadsheetApp.openById(sid);
　var last_row =2;
　var last_col = 2;
  var sname=data[0];
  ...
  var ibend=moment(ibe[0][0]);
  var ibeday=[];
  for(var i=1;i<mkdt.length;i++){
    ibeday[i] = moment(ibend-left[i]*3600000);
  }
  
  var tmp="";
  for(var i=1;i<ibeday.length;i++){
    if(ibeday[i].valueOf()==t.valueOf()){
      tmp=ibeday[i];
    }
  }
...
function onOpen() {
  ...
  menu.addToUi();                            // メニューをUiクラスに追加する var menu = ui.createMenu('ぼだJS振り分け');  // Uiクラスからメニューを作成する
  menu.addItem('くろーん', 'clonespreadsheet');   // メニューにアイテムを追加する
